...
final class EcjCompilerDiagnostics {
	...
	List<CategorizedProblem> diagnose(MemoryCompilationUnit unit) {
		return diagnose(unit, Collections.emptyMap(), BrowserCompilerConfiguration.DEFAULT);
	}

	List<CategorizedProblem> diagnose(MemoryCompilationUnit unit, Map<String, MemoryCompilationUnit> workspaceUnits) {
		return diagnose(unit, workspaceUnits, BrowserCompilerConfiguration.DEFAULT);
	}

	List<CategorizedProblem> diagnose(MemoryCompilationUnit unit, Map<String, MemoryCompilationUnit> workspaceUnits,
			BrowserCompilerConfiguration configuration) {
		CollectingRequestor requestor = new CollectingRequestor(new String(unit.getFileName()));
		NoCodegenCompiler compiler = new NoCodegenCompiler(new BrowserNameEnvironment(unit, workspaceUnits),
	...
	private static final class NoCodegenCompiler extends Compiler {
		NoCodegenCompiler(INameEnvironment environment, CompilerOptions options, ICompilerRequestor requestor) {
			super(environment, DefaultErrorHandlingPolicies.proceedWithAllProblems(), options, requestor,
					new EcjDiagnosticsEngine.BrowserProblemFactory());
		}
	...
	private static final class BrowserNameEnvironment implements INameEnvironment {
		...
		BrowserNameEnvironment(MemoryCompilationUnit userUnit, Map<String, MemoryCompilationUnit> workspaceUnits) {
			registerPackage("");
		...
		private NameEnvironmentAnswer find(String qualifiedName) {
			...
			return sourceType == null ? null : new NameEnvironmentAnswer(sourceType, null);
		}
